          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((p) => (
              <a
                key={p.slug}
                href={`/blog/${p.slug}`}
                className="group rounded-2xl border border-gray-800 hover:border-orange-500/50 hover:shadow-[0_0_30px_rgba(249,115,22,0.2)] transition-all duration-300 p-6 block bg-black/50 hover:bg-black/70"
              >
                <div className="text-xs text-gray-500 mb-3">{p.frontmatter.date || ''}</div>
                <h2 className="text-xl font-bold text-white mb-3 group-hover:text-orange-300 transition-colors leading-tight">
